<template>
    <div id="dashboard">
        ...
        <Modal type="m-add-loc" style="height: 200px !important;">
            <template slot="modal-title"><span v-if="!isEditClicked">Add</span><span v-else>Edit</span> Location</template>
            <template slot="modal-content">
                <div class="row">
                    <form class="col s12" id="add_loc_form" name="add_loc_form" @submit.prevent="updateLocation">
                        <div class="row">
                            <div class="col s2">
                                <div class="input-field">
                                    <input v-model="selLoc.name" type="text" name="name" id="name" disabled>
                                    <label for="name">Name</label>
                                </div>
                            </div>
                            <div class="col s12">
                                <div class="input-field">
                                    <input v-model="selLoc.title" type="text" name="title" id="title" required>
                                    <label for="title">Title</label>
                                </div>
                            </div>
                            <div class="col s6">
                                <div class="input-field">
                                    <input v-model="selLoc.long" type="text" name="long" id="long" required>
                                    <label for="long">Longitude</label>
                                </div>
                            </div>
                            <div class="col s6">
                                <div class="input-field">
                                    <input v-model="selLoc.lat" type="text" name="lat" id="lat" required>
            ...
            <template slot="modal-footer">
                ...
                <a class="waves-effect waves-green btn-flat green-text" onclick="$('#magic').click();"><span v-if="!isEditClicked">SAVE</span><span v-else>UPDATE</span></a>
            </template>
        </Modal>
        ...
        <Modal type="m-delete-loc" style="height: 250px !important;">
            ...
            <template slot="modal-footer">
                ...
                <a class="waves-effect waves-light btn-flat green-text modal-close" @click="deleteLocation">YES</a>
            </template>
        </Modal>
                        ...
                        <table class="highlight">
                            ...
                            <tbody>
                                <tr v-for="(location,i) in locations.data" :key="location.id">
                                    ...
                                    <td>{{location.msg.trim() != ""? location.msg : '--No Message--'}}</td>
                                    ...
                                    <!-- <td></td> -->
                                    <td><span class="orange-text text-dargen-4"><b>{{location.status}}</b></span></td>
                                    <td class="option">
                                        <a class="waves-effect waves-dark btn-flat" @click="flyToLocation(location.long, location.lat)"><i class="material-icons small blue-text text-darken-2">remove_red_eye</i></a>
                                        <a class="waves-effect waves-dark btn-flat" @click="openEditModal(location.id)" style="margin: auto 25px;"><i class="material-icons small orange-text text-darken-2">edit</i></a>
                                        <a class="waves-effect waves-dark btn-flat" @click="openDeleteModal(location.id)"><i class="material-icons small red-text">delete</i></a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <ul class="pagination">
                            <li :class="{disabled : locations.meta.current_page === 1}"><a @click="pagination(1,true)"><i class="material-icons">chevron_left</i></a></li>
                            <li class="waves-effect" :class="{active : a === locations.meta.current_page}" v-for="a in links" :key="a">
                                <a @click="pagination(a,false)">{{ a }}</a>
                            </li>
                            <li class="waves-effect" :class="{disabled : locations.meta.current_page === links}"><a @click="pagination(locations.meta.current_page + 1,true)"><i class="material-icons">chevron_right</i></a></li> 
                        </ul>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods:{
        pagination(page,isChevronClicked){
            if((this.link.next != null && isChevronClicked) && this.locations.meta.current_page != page || !isChevronClicked)
                this.$store.dispatch('FETCH_LOCATION_PAGE', page);
        },
        updateLocation(){
            if(!this.isEditClicked){
                /* ADD LOCATION */
                M.toast({html:'<div v-if="false" class="preloader-wrapper small active">'+
                    ...
                    '</div>'+
                    '</div>'+
                '</div><span style="margin-left: 10px;">Loading. . .</span>' ,classes: 'rounded', displayLength: 99999});
                ...
                /* EDIT LOCATION */
                axios.put(`/api/location`, this.selLoc).then(res=>{
                    $('.modal').modal('close');
        ...
        deleteLocation(){
            axios.delete(`/api/location/${this.selLoc.id}`).then(res=>{
        ...
        selectLocation(id){
            return this.$store.getters.locations.find(loc => loc.id == id);
        },
        ...
        updateMap(i,point){
            ...
            if(i == 1){
                ...
                /* UPDATE MAP REALTIME */
                const index = this.points.findIndex(p => p.geometry.coordinates[0] == parseFloat(point.long) && p.geometry.coordinates[1] == parseFloat(point.lat));
                this.points[index] = {
                    "type": "Feature",
                    "geometry": {
                        "type": "Point",
